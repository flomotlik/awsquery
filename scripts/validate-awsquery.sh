...
run_test "Filter: Multiple words" "$BASE_CMD ec2 describe-instances running attached"
run_test "Filter: With columns" "$BASE_CMD iam list-users test -- UserName CreateDate"
echo ""
...
run_test "Keys: With filters" "$BASE_CMD iam list-users test --keys"
echo ""
...
run_test "Multi-level: Function hint" "$BASE_CMD eks list-nodegroups -i list-clus:cluster"
...
run_test "Multi-level: With filters" "$BASE_CMD cloudformation describe-stack-events -- StackName ResourceStatus"
echo ""

echo "--- Resource Filters (before --) ---"
run_test "Resource filter: Basic" "$BASE_CMD cloudformation list-stack-resources worker"
run_test "Resource filter: Multiple" "$BASE_CMD eks list-nodegroups sandbox"
echo ""
...
run_test "Combined: -i + filter + columns" "$BASE_CMD cloudformation describe-stack-events worker -- StackName Timestamp"
