...
class TestAllFeaturesIntegration:
    ...
    def test_region_profile_with_default_columns(
        ...
        with patch.object(sys, 'argv', test_args), \
             patch('src.awsquery.cli.format_table_output') as mock_format:
    ...
    def test_keys_mode_with_session_and_transformed_tags(
        ...
        mock_create_session.assert_called_once_with(
            region='ap-southeast-2', profile='infra-admin'
        )
    ...
    def test_multi_level_with_session_defaults_and_tag_transformation(
        ...
        with patch('src.awsquery.cli.execute_aws_call') as mock_execute:
            ...
            test_args = [
                ...
                '--',
                'ACTIVE'      # value filter
                # No column filters - should use defaults if available
            ]
        ...
        # Verify session creation
        mock_create_session.assert_called_once_with(
            region='us-west-2', profile='k8s-admin'
        )
        ...
        assert 'ACTIVE' in value_filters
    ...
    def test_keys_mode_fallback_preserves_all_features(
        ...
        test_args = [
            ...
            'production',  # resource filter for multi-level resolution
            '--',
        ...
        mock_create_session.assert_called_once_with(
            region='ca-central-1', profile='db-admin'
        )
        ...
        mock_multi_level.assert_called_once()
        multi_call_args = mock_multi_level.call_args

        # Check that session was passed to multi-level
        # and filters were correctly parsed
        resource_filters = multi_call_args[0][2]
        value_filters = multi_call_args[0][3]
        assert 'production' in resource_filters
        assert 'available' in value_filters
...
class TestErrorHandlingAcrossFeatures:
    ...
    def test_session_error_with_other_features(
        ...
        test_args = [
            ...
            'running',
            '--',
            'InstanceId', 'Tags.Name'  # Default columns should still be parsed
        ]
...
class TestRegressionPrevention:
    ...
    def test_backward_compatibility_with_existing_commands(self):
        ...
        test_cases = [
            ...
            # Complex multi-level
            ['cloudformation', 'describe-stack-resources', 'prod', '--', 'CREATE_COMPLETE', '--', 'LogicalResourceId']
        ]
    ...
    @patch('src.awsquery.cli.load_security_policy')
    @patch('src.awsquery.cli.get_aws_services')
    def test_simple_commands_still_work(
        self, mock_services, mock_policy, mock_execute
    ):
        ...
        # Verify no session was created (default behavior)
        call_args = mock_execute.call_args
        # Session parameter should be None (default behavior)
        session_param = call_args[1].get('session')
        assert session_param is None