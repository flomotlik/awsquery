...
class TestSessionManagementIntegration:
    ...
    def test_multi_level_call_session_persistence(self, mock_session_class):
        ...
        final_response = {
            "cluster": {"name": "test-cluster", "status": "ACTIVE"}
        }
...
class TestSessionEnvironmentIntegration:
    ...
    def test_partial_environment_override(self):
        ...
        with patch.dict(os.environ, {'AWS_DEFAULT_REGION': 'us-west-1', 'AWS_PROFILE': 'env-profile'}):
            with patch('boto3.Session') as mock_session:
...
class TestCLISessionIntegration:
    ...
    def test_cli_region_profile_integration(
        ...
        # Session should be passed as keyword argument
        assert 'session' not in call_args[1] or call_args[1]['session'] == mock_session
