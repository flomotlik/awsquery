"""
Comprehensive CLI and end-to-end integration tests for AWS Query Tool.

This module provides comprehensive testing of CLI components and workflows including:
- CLI argument parsing and validation
- Security policy enforcement workflows
- Output formatting integration  
- Error handling scenarios
- Real-world usage patterns

Test coverage target: 80% for CLI functionality with 10-15 comprehensive tests.
"""
...
from src.awsquery.filters import parse_multi_level_filters
...
class TestEndToEndScenarios:
    """Complete end-to-end workflows testing CLI components integration."""
    
    def test_complete_aws_query_workflow_table_output(self, sample_ec2_response, mock_security_policy):
        """Test complete workflow: parsing -> security -> execution -> formatting."""
        # Test the complete integration workflow step by step
        
        # 1. Test argument parsing with multi-level filters
        argv = ['ec2', 'describe-instances', '--', 'InstanceId', 'State']
        base_cmd, res_filters, val_filters, col_filters = parse_multi_level_filters(argv)
        ...
        assert 'State' in table_output or 'Code' in table_output or 'Name' in table_output  # State shows as Code/Name
        assert 'i-1234567890abcdef0' in table_output
    
    def test_complete_aws_query_workflow_json_output(self, sample_ec2_response, mock_security_policy):
        """Test complete workflow with JSON output."""
        # Similar workflow but with JSON formatting
        
        # 1. Parse arguments (with JSON flag simulation)
        argv = ['ec2', 'describe-instances', '--json']  
        base_cmd, _, _, _ = parse_multi_level_filters(argv)
    ...
    def test_multi_level_cloudformation_workflow(self, sample_cloudformation_response, mock_security_policy):
        ...
        argv = ['cloudformation', 'describe-stack-resources', 'prod', '--', 'EC2', '--', 'StackName', 'ResourceType']
        base_cmd, res_filters, val_filters, col_filters = parse_multi_level_filters(argv)
    ...
    def test_output_format_integration_with_column_filtering(self, sample_ec2_response):
        ...
        assert 'Tags' in table_output or 'Tag' in table_output or 'Key' in table_output or 'Value' in table_output
...
class TestCLIArgumentParsing:
    ...
    def test_multi_level_filter_parsing_multiple_separators(self):
        ...
        # Test command: service action resource_filters -- value_filters -- column_filters
        argv = ['cf', 'describe-stack-resources', 'prod', '--', 'EKS', '--', 'StackName', 'ResourceType']
    ...
    def test_no_separator_parsing(self):
        ...
        assert base_command == ['s3', 'list-buckets', 'production']  # All args go to base_command when no separators
        assert resource_filters == []  # No separators means empty resource filters
    ...
    def test_autocomplete_service_completer(self, mock_session):
        ...
        mock_session_instance.get_available_services.return_value = [
            'ec2', 'ecs', 'eks', 's3', 'cloudformation', 'lambda', 'rds'
        ]
    ...
    def test_autocomplete_action_completer(self, mock_policy, mock_boto_client, mock_security_policy):
        ...
        mock_client.meta.service_model.operation_names = [
            'DescribeInstances', 'DescribeImages', 'DescribeSecurityGroups', 'TerminateInstances',
