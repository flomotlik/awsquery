"""
Comprehensive unit tests for AWS Query Tool core execution functions.

This module tests the core AWS API execution functions including:
- execute_aws_call() - Core AWS API execution with pagination support
- execute_multi_level_call() - Multi-level calls with parameter resolution  
- parse_validation_error() - Parse AWS validation errors
- infer_list_operation() - Infer list operations from parameter names
- extract_parameter_values() - Extract parameter values from resources
- get_correct_parameter_name() - Service model parameter introspection
- convert_parameter_name() - Convert parameter names to PascalCase
- parameter_expects_list() - Determine if parameter expects list or single value

Test coverage target: 95% for core functions
"""
...
class TestExecuteAwsCall:
    """Test suite for execute_aws_call() function."""
    
    def test_dry_run_mode_success(self, capsys):
        """Test dry run mode prints correctly and returns empty list."""
    ...
    def test_dry_run_mode_with_parameters(self, capsys):
        """Test dry run mode includes parameters in output."""
    ...
    def test_successful_paginated_call(self, mock_boto3, sample_ec2_response):
        """Test successful AWS call with pagination."""
    ...
    @patch('src.awsquery.core.boto3')
    @patch('src.awsquery.core.parse_validation_error')
    def test_client_error_validation_handling(self, mock_parse, mock_boto3, validation_error_fixtures):
    ...
    @pytest.mark.parametrize("action,expected_normalized", [
    ...
    @patch('src.awsquery.core.boto3')
    def test_action_name_normalization(self, mock_boto3, sample_ec2_response, action, expected_normalized):
...
class TestExecuteMultiLevelCall:
    ...
    def test_parameter_resolution_workflow(self, mock_get_param, mock_extract, mock_filter, 
                                          mock_flatten, mock_infer, mock_execute):
        ...
        mock_execute.side_effect = [
            {'validation_error': validation_error, 'original_error': Exception()},  # First call fails
    ...
    @patch('src.awsquery.formatters.flatten_response')
    @patch('src.awsquery.filters.filter_resources')
    def test_no_resources_after_filtering_exits(self, mock_filter, mock_flatten, mock_infer, mock_execute, capsys):
    ...
    @patch('src.awsquery.filters.filter_resources')
    @patch('src.awsquery.filters.extract_parameter_values')
    def test_no_parameter_values_extracted_exits(self, mock_extract, mock_filter, mock_flatten, 
                                                mock_infer, mock_execute, capsys):
        ...
        mock_execute.side_effect = [
            ...
            [{'Name': 'cluster1'}],  # List operation succeeds
            Exception("Should not reach this call")  # Should not get here since extract returns empty
        ]
        mock_infer.return_value = ['list_clusters']
        mock_flatten.return_value = [{'DifferentField': 'value1'}]  # Resources without the right field
        mock_filter.return_value = [{'DifferentField': 'value1'}]  # Resources without the right field 
    ...
    @patch('src.awsquery.core.get_correct_parameter_name')
    def test_multiple_parameter_values_handling(self, mock_get_param, mock_expects_list, mock_extract,
                                              mock_filter, mock_flatten, mock_infer, mock_execute, capsys):
        ...
        assert final_call == call('eks', 'describe-cluster', False, {'ClusterName': 'cluster1'})
    ...
    @patch('src.awsquery.core.parameter_expects_list')
    @patch('src.awsquery.core.get_correct_parameter_name')
    def test_persistent_validation_error_exits(self, mock_get_param, mock_expects_list, mock_extract,
                                             mock_filter, mock_flatten, mock_infer, mock_execute, capsys):
        ...
        mock_execute.side_effect = [
            ...
            [{'Name': 'cluster1'}],  # List operation succeeds  
            {'validation_error': persistent_error, 'original_error': Exception()}  # Final call still has error
        ]
        mock_infer.return_value = ['list_clusters']
        mock_flatten.return_value = [{'Name': 'cluster1'}]
...
class TestParameterResolution:
    ...
    @pytest.mark.parametrize("service,parameter,action,expected_operations", [
        (
            'ec2', 'instanceId', 'describe-instance-attribute',
            [
                'list_instances', 'describe_instances', 'get_instances',
                'list_instance', 'describe_instance', 'get_instance',
                'list_instance_attributes', 'describe_instance_attributes', 'get_instance_attributes',
                'list_instance_attribute', 'describe_instance_attribute', 'get_instance_attribute'
            ]
        ),
        (
            'eks', 'clusterName', 'describe-nodegroup',
            [
                'list_clusters', 'describe_clusters', 'get_clusters',
                'list_cluster', 'describe_cluster', 'get_cluster',
                'list_nodegroups', 'describe_nodegroups', 'get_nodegroups',
                'list_nodegroup', 'describe_nodegroup', 'get_nodegroup'
            ]
        ),
        (
            's3', 'bucketName', 'get-bucket-policy',
            [
                'list_buckets', 'describe_buckets', 'get_buckets',
                'list_bucket', 'describe_bucket', 'get_bucket',
                'list_bucket_policies', 'describe_bucket_policies', 'get_bucket_policies',
                'list_bucket_policy', 'describe_bucket_policy', 'get_bucket_policy'
            ]
        )
    ])
    def test_infer_list_operation_comprehensive(self, service, parameter, action, expected_operations):
    ...
    @pytest.mark.parametrize("parameter_name,expects_list", [
        ('instanceIds', True),
        ('clusterNames', True), 
        ('bucketArns', True),
        ('tagKeys', True),
    ...
    @pytest.mark.parametrize("parameter_name,expected", [
...
class TestResponseFlattening:
    ...
    def test_extract_parameter_values_partial_match(self):
        ...
        resources = [
            {'Cluster.ClusterName': 'cluster1'},
            {'Resource.ClusterName': 'cluster2'}
        ]
        result = extract_parameter_values(resources, 'ClusterName')
    ...
    @pytest.mark.parametrize("parameter,expected_standard_field", [
        ('bucketName', 'Name'),
        ('instanceId', 'Id'), 
        ('roleArn', 'Arn'),
        ('tagKey', 'Key'),
    ...
    def test_extract_parameter_values_standard_field_patterns(self, parameter, expected_standard_field):
