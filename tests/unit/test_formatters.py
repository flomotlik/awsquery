"""
Comprehensive unit tests for AWS Query Tool formatting functions.

This module tests the output formatting functions including:
- format_table_output() - Format resources as table using tabulate
- format_json_output() - Format resources as JSON output  
- flatten_response() - Flatten AWS response to extract resource lists
- flatten_single_response() - Simple extraction of data from AWS API responses
- flatten_dict_keys() - Flatten nested dictionary keys with dot notation
- simplify_key() - Extract the last non-numeric attribute from a flattened key
- extract_and_sort_keys() - Extract all keys from resources and sort them case-insensitively
- show_keys() - Show all available keys from API response

Test coverage target: 85% for formatting functions
"""
...
class TestFlattenResponse:
    """Test suite for flatten_response() function - handles paginated and single responses."""
    
    def test_flatten_response_empty_list(self):
        """Test flatten_response with empty list returns empty list."""
...
class TestSimplifyKey:
    ...
    @pytest.mark.parametrize("full_key,expected", [
        # Basic cases
        ("Name", "Name"),
...
class TestTableOutput:
    ...
    def test_format_table_output_long_value_truncation(self):
        ...
        resources = [
            {
                'ShortValue': 'short',
                'LongValue': 'a' * 60  # Over 50 character limit
            }
        ]
        result = format_table_output(resources)
    ...
    @pytest.mark.parametrize("column_filters,expected_columns", [
        (['name'], ['Name']),
...
class TestUtilityFunctions:
    ...
    def test_extract_and_sort_keys_case_insensitive_sort(self):
        ...
        resources = [
            {'zebra': 'z', 'Apple': 'a', 'Banana': 'b', 'cherry': 'c'}
        ]
        result = extract_and_sort_keys(resources)
    ...
    def test_extract_and_sort_keys_with_various_data_types(self):
        ...
        expected_keys = sorted(['ArrayField', 'BooleanField', 'NestedKey', 'NumberField', 'StringField'], key=str.lower)
        assert result == expected_keys
    ...
    def test_show_keys_dry_run(self, mock_execute):
        ...
        mock_execute.assert_called_once_with('ec2', 'describe-instances', True)
    ...
    def test_show_keys_integration(self, mock_execute):
        ...
        mock_execute.return_value = {
            'Instances': [
                {'InstanceId': 'i-123', 'Status': 'running'}
            ]
        }
...
class TestComplexScenarios:
    ...
    def test_format_table_output_complex_nested_structure(self):
        ...
        result = format_table_output([complex_resource], column_filters=['LoadBalancer', 'Protocol'])
    ...
    def test_format_json_output_mixed_data_types(self):
        ...
        result = format_json_output(resources, column_filters=['String', 'Number', 'Boolean', 'Key', 'Sub'])
        parsed = json.loads(result)
    ...
    @pytest.mark.parametrize("column_filter,resource_type", [
