"""
AWS response fixtures for comprehensive testing scenarios.

This module provides realistic AWS API responses for testing pagination,
complex nested structures, and various edge cases.
"""
...
def get_paginated_response(service='ec2', operation='describe_instances', num_pages=3, items_per_page=2):
    """
    ...
    if service == 'ec2' and operation == 'describe_instances':
        for page_num in range(num_pages):
            ...
            for item_num in range(items_per_page):
                ...
                instances.append({
                    ...
                    'Tags': [
                        ...
                        {'Key': 'Environment', 'Value': ['production', 'staging', 'development'][page_num % 3]},
                        {'Key': 'Team', 'Value': ['backend', 'frontend', 'devops'][item_num % 3]}
                    ],
                    'SecurityGroups': [
                        {
            ...
            page = {
                'Reservations': [
                    {
                        'ReservationId': f'r-{page_num:02d}{"e" * 15}',
                        'Instances': instances
                    }
                ]
            }
        ...
        for page_num in range(num_pages):
            ...
            for item_num in range(items_per_page):
                ...
                buckets.append({
                    'Name': bucket_name,
                    'CreationDate': datetime(2023, page_num + 1, item_num + 1, tzinfo=timezone.utc)
                })
        ...
        for page_num in range(num_pages):
            ...
            for item_num in range(items_per_page):
                ...
                stacks.append({
                    ...
                    'StackStatus': ['CREATE_COMPLETE', 'UPDATE_COMPLETE', 'ROLLBACK_COMPLETE'][item_num % 3],
                    'CreationTime': datetime(2023, page_num + 1, item_num + 1, tzinfo=timezone.utc),
                    'Parameters': [
                        {'ParameterKey': 'Environment', 'ParameterValue': ['prod', 'staging', 'dev'][page_num % 3]},
                        {'ParameterKey': 'InstanceType', 'ParameterValue': ['t3.medium', 't3.small', 't2.micro'][item_num % 3]}
                    ],
                    'Tags': [
                        {'Key': 'Environment', 'Value': ['production', 'staging', 'development'][page_num % 3]},
    ...
    def create_nested_structure(current_depth, max_depth, current_breadth):
        ...
        for j in range(current_breadth):
            array_item = {
                ...
                f'ArrayItem{j}Nested': create_nested_structure(current_depth + 1, max_depth, max(1, current_breadth - 1))
            }
